	...
	public int update(Student stu)
	{
		...
		//{
		   //NAME
			...
			if(stu.name.equals("") || stu.name.equals(" ") || stu.name.equals("    ") || stu.name.equals("      "))
			{
			...
			if ((stu.rno <= 0))
			{
				JOptionPane.showMessageDialog(null, "Rollno should not contain negative or Zero Value ");
			}
			...
			if (( stu.name.charAt(j)>='A' && stu.name.charAt(j)<='Z' ) && (stu.name.length() >2) && (stu.rno >= 0) && (stu.marks1 > 0) && (stu.marks1 < 100) && (stu.marks2 > 0) && (stu.marks2 < 100) && (stu.marks3 > 0) && (stu.marks3 < 100))
			{
				s = sf.openSession();
